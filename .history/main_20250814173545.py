...
@app.callback(
    Output("histplot","figure"), Output("meta","children"),
    Output("xlim-fixed","min"), Output("xlim-fixed","max"), Output("xlim-fixed","value"),
...
def update_hist(hoverData, clickData, xscale, mode,
    ...
    if isinstance(fit_on, (list, tuple)) and ("on" in fit_on):
        ...
        if got is not None:
            ...
            # 1) 计算每个 bin 的 μ 宽度（用于把 pdf → count 对齐）
            if src == "linear":
            ...
            # 2) 拟合（fit_and_predict 当前在其内部使用中位 Δμ 缩放为 count）
            fitres = fit_and_predict(mu_fit, cnt_fit, pct=float(fit_pct))
            if fitres is not None:
                x_fit = fitres["curves"]["x"]           # 拟合用的横轴（均匀网格）
                y_mix = fitres["curves"]["mix"]         # 已按中位 Δμ 转成了 counts 的曲线
                y_bulk = fitres["curves"]["bulk"]
                y_tail = fitres["curves"]["tail"]

                # 3) 将拟合曲线插值到直方图的 μ 中心（与 hist x 对齐）
                y_mix_i  = np.interp(mu_fit, x_fit, y_mix,  left=np.nan, right=np.nan)
                y_bulk_i = np.interp(mu_fit, x_fit, y_bulk, left=np.nan, right=np.nan)
                y_tail_i = np.interp(mu_fit, x_fit, y_tail, left=np.nan, right=np.nan)

                # 4) 用逐 bin 宽度修正（替换掉“中位 Δμ”的假设）
                dmu_assumed = np.median(np.diff(x_fit)) if len(x_fit) > 1 else 1.0
                scale = bin_widths / dmu_assumed
                y_mix_i  = y_mix_i  * scale
                y_bulk_i = y_bulk_i * scale
                y_tail_i = y_tail_i * scale

                # 5) log 轴下过滤 μ<=0
                if xscale == "log":
                    mpos = mu_fit > 0
                    mu_plot  = mu_fit[mpos]
                    y_mix_i  = y_mix_i[mpos]
                    y_bulk_i = y_bulk_i[mpos]
                    y_tail_i = y_tail_i[mpos]
                else:
                    mu_plot = mu_fit

                # 6) 叠加三条曲线（也可以只画 mix）
                fig.add_scatter(x=mu_plot, y=y_mix_i,  mode="lines", name="fit: mix",  line=dict(width=2))
                fig.add_scatter(x=mu_plot, y=y_bulk_i, mode="lines", name="fit: bulk", line=dict(width=1, dash="dot"))
                fig.add_scatter(x=mu_plot, y=y_tail_i, mode="lines", name="fit: tail", line=dict(width=1, dash="dash"))
